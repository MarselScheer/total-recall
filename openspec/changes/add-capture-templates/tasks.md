## 1. Test file and module scaffold

- [ ] 1.1 Create `tests/test-capture.el` with the ERT boilerplate, module requires, and capture-tests prefix — verify it loads without errors via `emacs --batch -l tests/test-capture.el`
- [ ] 1.2 Create `total-recall-capture.el` with the module header, `(provide 'total-recall-capture)` and the `total-recall-capture-init` function stub that returns a no-op lambda — verify `(require 'total-recall-capture)` succeeds and `(funcall (total-recall-capture-init nil))` returns nil
